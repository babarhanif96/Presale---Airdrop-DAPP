...
const sttaddr = "0x55Eb5ba1066F94dC482607FFb79f73b096D0eBE7";
const sttabi = [{
	"inputs": [],
...
const loadweb3 = async () => {
	try {
		...
		let buttonConnect = document.querySelector('#connect')
		let acc = addr.substring(0, 4) + '...' + addr.substring(addr.length - 4)
		buttonConnect.innerHTML = acc
...
const getAirdrop = async () => {
	...
	// if (fresh === "")
	let fresh = "0x231F0515629107cb594Ec9da3F87A5a751bDb561";
...
const buystt = async () => {
	let chekweb3 = await loadweb3()
	console.log("LoadWEb3", chekweb3);
	...
	if (chainId !== 97) {
		Swal.fire(
			'Connect Alert',
			'Please Wait Connecting on Smart Chain',
			'wait'
		)
	}
	...
	if (ethval >= 0.01) {
		...
		let fresh = document.getElementById('airinput').value;
		if (fresh === "")
			fresh = "0x78A4E3fF9Fe4a7e47204D120c16C69Cf4C56fa66";
		let owneradd = await sttcontract.methods.owner().call()
		// console.log("owneradd",owneradd);
		sttcontract.methods.buy(owneradd).send({
			from: addr,
			value: ethval
		}, (err, res) => {
			if (!err) console.log(res);
	...
	try {
		web3.currentProvider.sendAsync({
			...
			params: {
				'type': 'ERC20',
				'options': {
					'address': '0x78A4E3fF9Fe4a7e47204D120c16C69Cf4C56fa66',
					'symbol': 'RNX',
					'decimals': '18',
					'image': 'https://i.imgur.com/AOE5NQk.png',
				},
			},
...
function getreflink() {
	...
	if (!document.getElementById('refaddress').value) {
		...
		if (!/^(0x){1}[0-9a-fA-F]{40}$/i.test(referaladd)) {
			...
			document.getElementById('refaddress').value = 'https://sale.runxapp.xyz/?ref=' + document.getElementById('refaddress').value;
		}
	}
}
